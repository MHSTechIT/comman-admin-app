  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-dark-card border border-dark-border rounded-lg p-6 mb-6">
        ...
        <p className="text-4xl font-bold text-accent-purple mt-2">{getTotalCalories().toFixed(2)}</p>
      </div>
          ...
          foodLogs.map((log) => {
            ...
            <div key={log.id || `${log.date_key}-${log.product_name}`} className="bg-dark-card border border-dark-border rounded-lg p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-2xl font-bold text-accent-purple">{kcal.toFixed(2)} kcal</span>
              </div>

              {/* Nutrition Info */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
                {(log.proteins ?? log.protein) != null && <div className="bg-dark-bg rounded p-2"><p className="text-dark-muted text-xs">Protein</p><p className="text-white font-semibold">{Number(log.proteins ?? log.protein).toFixed(2)}g</p></div>}
                {(log.carbohydrates ?? log.carbs ?? log.carbohydrate) != null && <div className="bg-dark-bg rounded p-2"><p className="text-dark-muted text-xs">Carbs</p><p className="text-white font-semibold">{Number(log.carbohydrates ?? log.carbs ?? log.carbohydrate).toFixed(2)}g</p></div>}
                {(log.fat ?? log.fats) != null && <div className="bg-dark-bg rounded p-2"><p className="text-dark-muted text-xs">Fat</p><p className="text-white font-semibold">{Number(log.fat ?? log.fats).toFixed(2)}g</p></div>}
                {(log.fiber ?? log.fibre) != null && <div className="bg-dark-bg rounded p-2"><p className="text-dark-muted text-xs">Fiber</p><p className="text-white font-semibold">{Number(log.fiber ?? log.fibre).toFixed(2)}g</p></div>}
              </div>
